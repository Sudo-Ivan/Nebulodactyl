...
type DockerNetworkConfiguration struct {
	...
	// defined.
	Name       string                  `default:"comet_nw"`
	ISPN       bool                    `default:"false" yaml:"ispn"`
	Driver     string                  `default:"bridge"`
	Mode       string                  `default:"comet_nw" yaml:"network_mode"`
...
type DockerConfiguration struct {
	// Host is the address of the container engine socket. When empty the
	// DOCKER_HOST environment variable is used, falling back to the engine
	// default. Podman is supported through its Docker compatible socket, for
	// example unix:///run/user/1000/podman/podman.sock for a rootless engine.
	Host string `default:"" json:"host" yaml:"host"`
	...
	// at the engine default. Comet historically set 1024, which cgroup v2 converts
	...
	// if the value is "host", then the server containers are started with user namespace
	// remapping disabled
