...
// exposed by the Comet webserver.
...
// from Comet to the Panel.
type RemoteQueryConfiguration struct {
	// The amount of time in seconds that Comet should allow for a request to the Panel API
	...
	// Comet instance. A single request is initially made to the Panel to get this number
...
type SystemConfiguration struct {
	// The root directory where all of the comet data is stored at.
	RootDirectory string `default:"/var/lib/comet" json:"-" yaml:"root_directory"`

	// Directory where logs for server installations and other comet events are logged.
	LogDirectory string `default:"/var/log/comet" json:"-" yaml:"log_directory"`

	// Directory where the server data is stored at.
	Data string `default:"/var/lib/comet/volumes" json:"-" yaml:"data"`

	// Directory where server archives for transferring will be stored.
	ArchiveDirectory string `default:"/var/lib/comet/archives" json:"-" yaml:"archive_directory"`

	// Directory where local backups will be stored on the machine.
	BackupDirectory string `default:"/var/lib/comet/backups" json:"-" yaml:"backup_directory"`

	// TmpDirectory specifies where temporary files for Pterodactyl installation processes
	// should be created. This supports environments running docker-in-docker.
	TmpDirectory string `default:"/tmp/comet" json:"-" yaml:"tmp_directory"`

	// The user that should own all of the server files, and be used for containers.
	Username string `default:"comet" yaml:"username"`

	// The timezone for this Comet instance. This is detected by Comet automatically if possible,
	// and falls back to UTC if not able to be detected. If you need to set this manually, that
	// can also be done.
	//
	// This timezone value is passed into all containers created by Comet.
	...
	// Passwd controls the mounting of a generated passwd files into containers started by Comet.
	Passwd struct {
		// Enable controls whether generated passwd files should be mounted into containers.
		//
		// By default this option is disabled and Comet will not mount any
		...
		// This directory may be temporary as it will be re-created whenever Comet is started.
		//
		// This path **WILL** be both written to by Comet and mounted into containers created by
		// Comet. If you are running Comet itself in a container, this path will need to be mounted
		// into the Comet container as the exact path on the host, which should match the value
		...
		Directory string `yaml:"directory" default:"/run/comet/etc"`
	} `yaml:"passwd"`

	// MachineID controls the mounting of a generated `/etc/machine-id` file into containers started by Comet.
	MachineID struct {
		// Enable controls whether a generated machine-id file should be mounted
		// into containers.
		//
		// By default this option is enabled and Comet will mount an additional
		...
		// specified here. If you are using SELinux, you will need to make sure this file has the
		// correct SELinux context in order for containers to use it.
		Directory string `yaml:"directory" default:"/run/comet/machine-id"`
	} `yaml:"machine_id"`

	// The amount of time in seconds that can elapse before a server's disk space calculation is
	// considered stale and a re-check should occur. DANGER: setting this value too low can seriously
	// impact system performance and cause massive I/O bottlenecks and high CPU usage for the Comet
	// process.
	//
	...
	// If set to false Comet will not attempt to write a log rotate configuration to the disk
...
type CrashDetection struct {
	...
	// Determines if Comet should detect a server that stops with a normal exit code of
	// "0" as being crashed if the process stopped without any Comet interaction. E.g.
...
type Backups struct {
	...
	// CompressionLevel determines how much backups created by comet should be compressed.
	//
...
type Configuration struct {
	...
	// Determines if comet should be running in debug mode. This value is ignored
	// if the debug flag is passed through the command line arguments.
	Debug bool

	AppName string `default:"Nebulodactyl" json:"app_name" yaml:"app_name"`
	...
	// is only required by users running Comet without SSL certificates and using internal IP
...
// configuration update cannot leave Comet and the Panel using different keys.
func (c *Configuration) ResolveToken(remote bool) error {
	...
	var err error
	if c.Token.ID, err = resolve("token ID", os.Getenv("COMET_TOKEN_ID"), c.AuthenticationTokenId); err != nil {
		return err
	}
	if c.Token.Token, err = resolve("token", os.Getenv("COMET_TOKEN"), c.AuthenticationToken); err != nil {
		return err
	}
	return nil
}
...
// EnsureCometUser ensures that the Pterodactyl core user exists on the
...
func EnsureCometUser() error {
	sysName, err := getSystemName()
	if err != nil {
		return err
	}

	// Our way of detecting if comet is running inside of Docker.
	if sysName == "distroless" {
		_config.System.Username = system.FirstNotEmpty(os.Getenv("COMET_USERNAME"), "comet")
		_config.System.User.Uid = system.MustInt(system.FirstNotEmpty(os.Getenv("COMET_UID"), "988"))
		_config.System.User.Gid = system.MustInt(system.FirstNotEmpty(os.Getenv("COMET_GID"), "988"))
		return nil
	}
	...
	log.WithField("username", _config.System.Username).Info("checking for comet system user")
...
// ConfigurePasswd generates required passwd files for use with containers started by Comet.
...
// EnableLogRotation writes a logrotate file for comet to the system logrotate
...
func EnableLogRotation() error {
	if !_config.System.EnableLogRotate {
		log.Info("skipping log rotate configuration, disabled in comet config file")
		return nil
	}
	...
	if _, err := os.Stat("/etc/logrotate.d/comet"); err == nil || !os.IsNotExist(err) {
		return err
	}
	...
	// but there is not a file for comet already. In that case, let us write a new file to
	// it so files can be rotated easily.
	f, err := os.Create("/etc/logrotate.d/comet")
	if err != nil {
		return err
	}
	defer f.Close()

	t, err := template.New("logrotate").Parse(`{{.LogDirectory}}/comet.log {
    size 10M
    compress
    delaycompress
    dateext
    ...
    notifempty
    postrotate
        /usr/bin/systemctl kill -s HUP comet.service >/dev/null 2>&1 || true
    endscript
}`)
	if err != nil {
		return err
...
func ConfigureTimezone() error {
	...
	if _config.System.Timezone == "" {
		...
		if err != nil {
			...
			// fall through to UTC to get Comet booted at least.
