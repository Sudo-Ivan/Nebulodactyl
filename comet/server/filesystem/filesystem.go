...
func (fs *Filesystem) Write(p string, r io.Reader, newSize int64, mode ufs.FileMode) error {
	...
	// implicitly, but as the user Comet runs as; creating them here lets us
...
// WriteAt writes the contents of r to the file at path p starting at the
// given byte offset. If offset is zero the file is truncated before writing,
// matching the behavior of Write. If the offset is less than the current file
// size the file is truncated to the offset before appending. An offset beyond
// the current file size is rejected since sparse files would break the disk
// usage accounting.
//
...
func (fs *Filesystem) WriteAt(p string, r io.Reader, offset int64, mode ufs.FileMode) (int64, error) {
	...
	st, err := fs.unixFS.Stat(p)
	if err != nil && !errors.Is(err, ufs.ErrNotExist) {
		return 0, errors.Wrap(err, "server/filesystem: writeat: failed to stat file")
	} else if err == nil {
		if st.IsDir() {
			return 0, errors.WithStack(&Error{code: ErrCodeIsDirectory, resolved: ""})
		}
		currentSize = st.Size()
	}

	if offset < 0 || offset > currentSize {
		return 0, errors.WithStack(&Error{code: ErrCodePathResolution, resolved: "offset beyond end of file"})
	}
	...
	if offset < currentSize {
		if err := file.Truncate(offset); err != nil {
			return 0, fmt.Errorf("error truncating file: %w", err)
		}
	}
	...
	if err := fs.chownFile(p); err != nil {
		return n, fmt.Errorf("error chowning file: %w", err)
	}
	return n, nil
}

// Size returns the size in bytes of the file at path p. A non-existent file
// reports a size of zero with no error.
...
func (fs *Filesystem) Chown(p string) error {
	...
	// This walker is probably some of the most efficient code in Comet. It has
