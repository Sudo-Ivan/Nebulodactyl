...
import (
	...
	"github.com/Sudo-Ivan/Nebulodactyl/comet/internal/ufs"
...
func NewFs() (*Filesystem, *rootFs) {
	...
	tmpDir, err := os.MkdirTemp(os.TempDir(), "comet")
	if err != nil {
		panic(err)
		return nil, nil
	}
...
func TestFilesystem_WriteAt(t *testing.T) {
	g := Goblin(t)
	fs, _ := NewFs()

	g.Describe("WriteAt", func() {
		g.It("writes a file in chunks at increasing offsets", func() {
			n, err := fs.WriteAt("chunked.bin", bytes.NewReader([]byte("hello ")), 0, 0o644)
			g.Assert(err).IsNil()
			g.Assert(n).Equal(int64(6))

			n, err = fs.WriteAt("chunked.bin", bytes.NewReader([]byte("world")), 6, 0o644)
			g.Assert(err).IsNil()
			g.Assert(n).Equal(int64(5))

			f, st, err := fs.File("chunked.bin")
			g.Assert(err).IsNil()
			defer f.Close()
			g.Assert(st.Size()).Equal(int64(11))
			g.Assert(getFileContent(f)).Equal("hello world")
		})

		g.It("truncates and rewrites when the offset is before the end", func() {
			err := fs.Write("data.bin", bytes.NewReader([]byte("aaaaaaaaaa")), 10, 0o644)
			g.Assert(err).IsNil()

			n, err := fs.WriteAt("data.bin", bytes.NewReader([]byte("bbbb")), 3, 0o644)
			g.Assert(err).IsNil()
			g.Assert(n).Equal(int64(4))

			f, _, err := fs.File("data.bin")
			g.Assert(err).IsNil()
			defer f.Close()
			g.Assert(getFileContent(f)).Equal("aaabbbb")
		})

		g.It("rejects an offset beyond the current file size", func() {
			err := fs.Write("small.bin", bytes.NewReader([]byte("abc")), 3, 0o644)
			g.Assert(err).IsNil()

			_, err = fs.WriteAt("small.bin", bytes.NewReader([]byte("x")), 10, 0o644)
			g.Assert(err).IsNotNil()

			_, err = fs.WriteAt("missing.bin", bytes.NewReader([]byte("x")), 5, 0o644)
			g.Assert(err).IsNotNil()
		})

		g.It("rejects writing to a directory", func() {
			err := fs.CreateDirectory("subdir", "/")
			g.Assert(err).IsNil()

			_, err = fs.WriteAt("subdir", bytes.NewReader([]byte("x")), 0, 0o644)
			g.Assert(err).IsNotNil()
		})
		...
		g.It("reports the stored size for resume discovery", func() {
			...
			g.Assert(err).IsNil()
			g.Assert(size).Equal(int64(0))

			err = fs.Write("exists.bin", bytes.NewReader([]byte("12345")), 5, 0o644)
			g.Assert(err).IsNil()

			size, err = fs.Size("exists.bin")
			g.Assert(err).IsNil()
			g.Assert(size).Equal(int64(5))
		})
