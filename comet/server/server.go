...
// Sync syncs the state of the server on the Panel with Comet. This ensures that
// we're always using the state of the server from the Panel and allows us to
// not require successful API calls to Comet to do things.
//
// This also means mass actions can be performed against servers on the Panel
// and they will automatically sync with Comet when the server is started.
...
// instance on Comet. This includes the information needed by the Panel in order
