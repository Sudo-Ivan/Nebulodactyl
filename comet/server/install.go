...
func (ip *InstallationProcess) AfterExecute(containerId string) error {
	...
	tmpl, err := template.New("header").Parse(`Nebulodactyl Server Installation Log
...
func (ip *InstallationProcess) Execute() (string, error) {
	...
	// not exist when this runs if Comet boots with a missing directory and a user
