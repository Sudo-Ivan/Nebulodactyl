...
func newDiagnosticsCommand() *cobra.Command {
	command := &cobra.Command{
		Use:   "diagnostics",
		Short: "Collect and report information about this Comet instance to assist in debugging.",
...
// diagnosticsCmdRun collects diagnostics about comet, its configuration and the node.
// We collect:
// - comet and docker versions
...
func diagnosticsCmdRun(*cobra.Command, []string) {
	...
	fmt.Fprintln(output, "Nebulodactyl Comet - Diagnostics Report")
	printHeader(output, "Versions")
	fmt.Fprintln(output, "               Comet:", system.Version)
	if dockerErr == nil {
	...
	printHeader(output, "Latest Comet Logs")
	if diagnosticsArgs.IncludeLogs {
		p := "/var/log/comet/comet.log"
		if cfg != nil {
			p = path.Join(cfg.System.LogDirectory, "comet.log")
		}
