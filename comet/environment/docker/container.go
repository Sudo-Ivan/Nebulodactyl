...
func (e *Environment) Create() error {
	...
	for i, v := range evs {
		// Convert 127.0.0.1 to the comet0 network interface if the environment is Docker
	...
	if a.ForceOutgoingIP {
		...
		if _, err := e.client.NetworkInspect(ctx, networkName, network.InspectOptions{}); err != nil {
			...
			if environment.IsPodman() {
				e.log().Warn("environment/docker: podman does not support pinning outgoing IPs, network created without host_ipv4")
			} else {
	...
	hostConf := &container.HostConfig{
		...
		LogConfig: environment.LogConfig(cfg.Docker.ContainerLogConfig()),
...
func (e *Environment) SendCommand(c string) error {
	...
	// stop and Comet will think it has crashed and attempt to restart it.
