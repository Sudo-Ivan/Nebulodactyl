...
import (
	...
	"github.com/Sudo-Ivan/Nebulodactyl/comet/router/downloader"
...
func postServerDecompressFiles(c *gin.Context) {
	...
	if err != nil {
		if filesystem.IsErrorCode(err, filesystem.ErrCodeUnknownArchive) {
			...
			c.AbortWithStatusJSON(http.StatusBadRequest, gin.H{"error": "The archive provided is in a format Comet does not understand."})
			return
		}
		middleware.CaptureAndAbort(c, err)
		return
...
// extractUploadToken validates the upload token and returns the server it
// applies to. Unlike the multipart upload endpoint, chunked uploads reuse the
// token across requests so the unique-request check is not applied here. The
// token still requires a valid signature, the file upload scope and an
// unexpired lifetime.
func extractUploadToken(c *gin.Context) (*server.Server, *tokens.UploadPayload, bool) {
	manager := middleware.ExtractManager(c)
...
// headServerUploadFile reports how many bytes of the target file have already
// been stored, allowing a client to resume an interrupted upload. The offset
// is returned in the X-Upload-Offset header.
func headServerUploadFile(c *gin.Context) {
	s, _, ok := extractUploadToken(c)
	if !ok {
		return
	}
	...
	c.Header("X-Upload-Offset", strconv.FormatInt(size, 10))
	c.Header("Accept-Ranges", "bytes")
	c.Status(http.StatusNoContent)
}

// putServerUploadFile writes a raw request body to a file at the offset given
// by the Content-Range header. This provides fast resumable uploads: clients
// send bytes start-end/total ranges and can resume from the offset reported
// by HEAD after an interruption.
func putServerUploadFile(c *gin.Context) {
	s, token, ok := extractUploadToken(c)
	if !ok {
		return
	}
	...
	if cr := c.GetHeader("Content-Range"); cr != "" {
		var u string
		if n, err := fmt.Sscanf(cr, "bytes %d-%d/%s", &start, &end, &u); err != nil || n != 3 {
			c.AbortWithStatusJSON(http.StatusBadRequest, gin.H{
				"error": "Invalid Content-Range header, expected bytes start-end/total.",
			})
			return
		}
		if u != "*" {
			var err error
			if total, err = strconv.ParseInt(u, 10, 64); err != nil {
				c.AbortWithStatusJSON(http.StatusBadRequest, gin.H{
					"error": "Invalid Content-Range total.",
				})
				return
			}
		}
		if start < 0 || end < start || (total >= 0 && end >= total) {
			c.AbortWithStatusJSON(http.StatusRequestedRangeNotSatisfiable, gin.H{
				"error": "Invalid Content-Range values.",
			})
			return
		}
	}
	...
	p := filepath.Join(c.Query("directory"), c.Query("name"))
	if p == "." || strings.HasSuffix(p, "/") || c.Query("name") == "" {
		c.AbortWithStatusJSON(http.StatusBadRequest, gin.H{
			"error": "A file name must be provided.",
		})
		return
	...
	c.Header("X-Upload-Offset", strconv.FormatInt(start+n, 10))
	s.SaveActivity(s.NewRequestActivity(token.UserUuid, c.ClientIP()), server.ActivityFileUploaded, models.ActivityMeta{
		"file":      filepath.Base(p),
		"directory": filepath.Clean(c.Query("directory")),
	})
	c.Status(http.StatusNoContent)
}
