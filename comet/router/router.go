...
import (
	...
	wserver "github.com/Sudo-Ivan/Nebulodactyl/comet/server"
)
...
func Configure(m *wserver.Manager, client remote.Client) *gin.Engine {
	...
	// Resumable raw upload endpoint. Clients discover the stored offset with
	// HEAD and stream chunks at a byte offset with PUT and a Content-Range
	// header.
	router.HEAD("/upload/file", headServerUploadFile)
	router.PUT("/upload/file", putServerUploadFile)
	...
	server.Use(middleware.RequireAuthorization(), middleware.ServerExists())
	{
		...
		backup := server.Group("/backup")
		{
			...
			backup.HEAD("/:backup", headServerBackup)
