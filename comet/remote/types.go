...
import (
	...
	"github.com/Sudo-Ivan/Nebulodactyl/comet/parser"
)
...
// the Panel. When a server process is started, Comet communicates with the
// Panel to fetch the latest build information as well as get all the details
// needed to parse the given Egg.
//
// This means we do not need to hit Comet each time part of the server is
// updated, and the Panel serves as the source of truth at all times. This also
// means if a configuration is accidentally wiped on Comet we can self-recover
// without too much hassle, so long as Comet is aware of what servers should
// exist on it.
...
// when determining if the credentials provided to Comet are valid.
...
// instance. This sets what Comet is looking for to mark a server as done
